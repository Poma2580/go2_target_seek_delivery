"""Launch online RTAB-Map mapping and Nav2 static-goal navigation for go2_3.
...
def generate_launch_description():
    ...
    rtabmap_config = os.path.join(
        config_share, "config", "rtabmap", "go2_3_mapping.yaml"
    )
    nav2_config = os.path.join(config_share, "config", "nav2", "go2_3_nav2.yaml")
    rviz_config = os.path.join(config_share, "rviz", "go2_3_mapping_nav.rviz")
    ...
    return LaunchDescription(
        ...
        + [
            ...
            DeclareLaunchArgument(
                ...
                if delivery_root
                else str(runtime_root / "runtime" / "maps" / "go2_3_mapping.db"),
            ),
            DeclareLaunchArgument(
                "cloud_topic", default_value="/go2_3/velodyne_points"
            ),
            DeclareLaunchArgument("scan_topic", default_value="/go2_3/scan"),
            DeclareLaunchArgument("odom_topic", default_value="/go2_3/odom"),
            DeclareLaunchArgument("map_topic", default_value="/go2_3/map"),
            DeclareLaunchArgument("cmd_vel_topic", default_value="/go2_3/cmd_vel"),
            DeclareLaunchArgument("raw_cmd_vel_topic", default_value="/go2_3/raw_cmd_nav_vel"),
            ...
            Node(
                ...
                name="go2_3_pointcloud_to_laserscan",
                output="screen",
                parameters=[
                    {
                        "use_sim_time": use_sim_time_parameter,
            ...
            Node(
                ...
                name="go2_3_map_to_odom",
                output="screen",
                condition=IfCondition(LaunchConfiguration("publish_map_to_odom_tf")),
                arguments=["0", "0", "0", "0", "0", "0", "go2_3/map", "go2_3/odom"],
            ),
            Node(
                package="tf2_ros",
                executable="static_transform_publisher",
                name="go2_3_base_footprint_to_base_link",
                output="screen",
