...
class GazeboMapCreator : public gazebo::SystemPlugin
{
                      ...
                      std::shared_ptr<gazebo_map_creator_interface::srv::MapRequest::Response>  _res )
  {
    ...
    const double size_x = _req->upperleft.x - _req->lowerright.x;
    const double size_y = _req->lowerright.y - _req->upperleft.y;  // size_y is negative
    const double size_z = _req->upperleft.z - _req->lowerright.z;

    // Check for coordinate validity
    if (size_x <= 0 || size_y >= 0 || size_z <= 0 || _req->resolution <= 0.0)
    {
    ...
    // Occupancy maps describe cells, not inclusive sample endpoints.  The
    // upstream implementation added one to each dimension and sampled from
    // the lower edge.  That made the physical spacing disagree with the YAML
    // resolution and shifted world-to-grid conversion.  Use one sample per
    // requested cell and place it at the cell centre instead.
    int num_points_x = static_cast<int>(std::ceil(std::abs(size_x) / _req->resolution));
    int num_points_y = static_cast<int>(std::ceil(std::abs(size_y) / _req->resolution));
    int num_points_z = static_cast<int>(std::ceil(std::abs(size_z) / _req->resolution));

    // Calculate the step size in each dimension
    const double step_x = _req->resolution;
    const double step_y = -_req->resolution;
    double step_z = size_z / num_points_z;
    ...
    for (int x = 0; x < num_points_x; ++x)
    {
        ...
        const double cur_x = _req->lowerright.x + (x + 0.5) * step_x;
        for (int y = 0; y < num_points_y; ++y)
        {
            const double cur_y = _req->upperleft.y + (y + 0.5) * step_y;
