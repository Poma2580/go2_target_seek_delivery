"""
Run the five-stage curriculum for follower_slot_tracking_v0.

Each stage continues from the previous stage's best_model.pt. The environment
keeps the same observation layout across all stages.
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated total timesteps per stage, e.g. 200000,300000,400000,500000,600000",
    )
...
def parse_stage_steps(text):
    ...
    if len(values) != 5:
        raise ValueError("--stage-steps must contain exactly five comma-separated integers")
    return {stage: values[stage - 1] for stage in range(1, 6)}
...
def main():
    ...
    for stage in range(curriculum_args.start_stage, curriculum_args.end_stage + 1):
        print(f"\n===== Curriculum stage {stage} / {curriculum_args.end_stage} =====")
