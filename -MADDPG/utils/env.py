...
from envs.formation_navigation_v0 import FormationNavigationEnv
from envs.follower_slot_tracking_v0 import FollowerSlotTrackingEnv
...
ENV_MAP = {
    ...
    "formation_navigation_v0": FormationNavigationEnv,
    "follower_slot_tracking_v0": FollowerSlotTrackingEnv,
    "leader_slot_tracking_v0": LeaderSlotTrackingEnv,
}

CUSTOM_ENV_NAMES = {"formation_navigation_v0", "follower_slot_tracking_v0", "leader_slot_tracking_v0"}

def make_env(env_name, max_cycles=25, training_stage=4):
    """
    ...
    if env_name in CUSTOM_ENV_NAMES:
        env = ENV_MAP[env_name](max_cycles=max_cycles, render_mode="rgb_array", training_stage=training_stage)
    else:
...
def get_env_info(env_name, max_steps=25, seed=0, apply_padding=True, training_stage=4):
    """
    ...
    if env_name in CUSTOM_ENV_NAMES:
        env_single = ENV_MAP[env_name](max_cycles=max_steps, training_stage=training_stage)
    else:
        # 保留原 MPE 环境的调用方式（以防你以后还用）
        env_single = ENV_MAP[env_name].parallel_env(max_cycles=max_steps, continuous_actions=True)

    try:
        # 适配不同环境的reset调用（带seed参数）
        obs, infos = env_single.reset(seed=seed)
    except TypeError as e:
        # 兼容旧版PettingZoo（若reset不支持seed参数）
        print(f"Warning: reset with seed failed - {e}, trying without seed")
        obs, infos = env_single.reset()
...
def create_parallel_env(env_name, max_steps=25, num_envs=4):
    ...
    env = ss.concat_vec_envs_v1(
        make_env(env_name=env_name, max_cycles=max_steps, training_stage=4),
...
def create_single_env(env_name, max_steps=25, render_mode="rgb_array", apply_padding=True, training_stage=4):
    """
    ...
    if env_name in CUSTOM_ENV_NAMES:
        env = ENV_MAP[env_name](max_cycles=max_steps, render_mode=render_mode, training_stage=training_stage)
    else:
        # 保留原 MPE 环境的调用方式
        env = ENV_MAP[env_name].parallel_env(max_cycles=max_steps, continuous_actions=True, render_mode=render_mode)
